...
eu_drpw93_00 <- merge(cbp90czindp, eu_delta_rpw93_00, by = "isic")
...
eu_drpw01_10 <- merge(cbp90czindp, eu_delta_rpw01_10, by = "isic")
...
eu_drpw11_16 <- merge(cbp90czindp, eu_delta_rpw11_16, by = "isic")



### US Change Robots Per Worker ###

us_drpw04_10 <- us_delta_rpw('2004','2010','1990')
us_drpw11_16 <- us_delta_rpw('2011','2016','1990')


### Delta Robots Per Worker DataFrames ###

drpw90 <- merge(eu_drpw93_00, us_drpw04_10, by = "isic") %>% arrange(czone)
drpw00 <- merge(eu_drpw01_10, us_drpw04_10, by = "isic") %>% arrange(czone)
drpw10 <- merge(eu_drpw11_16, us_drpw11_16, by = "isic") %>% arrange(czone)
...
colnames(china_imp90) <- c("isic", "china_deltaip")

mex_imp90 <- delta_impPen(mex_trade,
                          begin = '1993',
                          end = '2000')
colnames(mex_imp90) <- c("isic", "mex_deltaip")

imp_pen90 <- merge(china_imp90, mex_imp90, by = "isic")
...
colnames(china_imp00) <- c("isic", "china_deltaip")

mex_imp00 <- delta_impPen(mex_trade,
                          begin = '2001',
                          end = '2010')
colnames(mex_imp00) <- c("isic", "mex_deltaip")

imp_pen00 <- merge(china_imp00, mex_imp00, by = "isic")
...
colnames(china_imp10) <- c("isic", "china_deltaip")

mex_imp10 <- delta_impPen(mex_trade,
                          begin = '1993',
                          end = '2000')
colnames(mex_imp10) <- c("isic", "mex_deltaip")

imp_pen10 <- merge(china_imp10, mex_imp10, by = "isic")
...
fsdf90 <- merge(drpw90, imp_pen90, by = "isic", all.x = T)
fsdf90[is.na(fsdf90$china_deltaip),c("china_deltaip", "mex_deltaip")] = 0
fsdf90 <- merge(fsdf90, demo_90c, by = "czone")
fsdf90 <- merge(fsdf90, ecc90, by = c("czone","fipscty"))
fsdf90 <- merge(fsdf90, pol_90, by = "fipscty") %>%
  select(czone, isic, dist_id, fipscty, fipstate,
         delta_nom, delta_rpw, eu_delta_rpw,
         everything()) %>%
  arrange(dist_id, fipscty, czone, isic)

# write.dta(fsdf90, "../outputs/data/first_stage/fsdf90.dta")

# 2000's Census


fsdf00 <- merge(drpw00, imp_pen00, by = "isic", all.x = T)
fsdf00[is.na(fsdf00$china_deltaip),c("china_deltaip", "mex_deltaip")] = 0
fsdf00 <- merge(fsdf00, demo_00c, by = "czone")
fsdf00 <- merge(fsdf00, ecc00, by = c("czone","fipscty"))
fsdf00 <- merge(fsdf00, pol_00, by = "fipscty") %>%
  select(czone, isic, dist_id, fipscty, fipstate,
         delta_nom, delta_rpw, eu_delta_rpw,
         everything()) %>%
  arrange(dist_id, fipscty, czone, isic)

# write.dta(fsdf00, "../outputs/data/first_stage/fsdf00.dta")

# 2010's Census


fsdf10 <- merge(drpw10, imp_pen10, by = "isic", all.x = T)
fsdf10[is.na(fsdf10$china_deltaip),c("china_deltaip", "mex_deltaip")] = 0
fsdf10 <- merge(fsdf10, demo_10c, by = "czone")
fsdf10 <- merge(fsdf10, ecc10, by = c("czone","fipscty"))
fsdf10 <- merge(fsdf10, pol_10, by = "fipscty") %>%
  select(czone, isic, dist_id, fipscty, fipstate,
         delta_nom, delta_rpw, eu_delta_rpw,
         everything()) %>%
  arrange(dist_id, fipscty, czone, isic)

# write.dta(fsdf10, "../outputs/data/first_stage/fsdf10.dta")
...
fsdf <- rbind(fsdf90, fsdf00)
fsdf <- rbind(fsdf, fsdf10) %>%
  arrange(czone, fipstate, district_code) %>%
  select(czone, fipstate, district_code, fipscty,
         census, isic,  t1, t2, t3, everything())

fsdf$beg_party <- "Democrat"
fsdf$beg_party[fsdf$gop_win == 1] = "GOP"

write.dta(fsdf, "../outputs/data/fsdf.dta")
